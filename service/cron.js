...
processStore = function (app_id) {
    var job = new CronJob({
        cronTime: '00 40 11 * * 0-6', //Runs every weekday (sunday to saturday) at 11:30:00 PM.
        onTick: function () {
            app_urls.findOneAndUpdate({APP_ID: app_id}, {$set: {prefetch_status: 'NOT STARTED'}}, function (err, user) {
                if (err) {
                    console.log(err)
                }
                if (!user) {
                    ...
                    console.log("prefetch_status update successfully");
                }
            })
        },
        start: false,
...
//                                                        FUNCTION CALLED FOR GETTING CATEGORY LIST
                                                        fetchCategoryList(prefetchDataDB, categoriesDB, app_id, URL, respond.msg.data.store_id, function () {
